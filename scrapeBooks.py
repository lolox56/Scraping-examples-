...
def scrape_books(html_soup, url):
    for book in html_soup.select('article.product_pod'):
        # Only store the books url for now
...
def scrape_book(html_soup, book_id):
    ...
    book['description'] = desc.find_next_sibling('p').get_text(strip=True) if desc else ''
    info_table = html_soup.find(string='Procuct Information').find_next('table')
    for row in info_table.findAll('tr'):
        header = row.find('th').get_text(strip=True)
        # Since we'll use the header as a column, clean a bit to make sure SQL accepts it:
...
for book in books:
    ...
    r = requests.get(book_url, verify=False)
    r.encoding = 'utf-8'
    html_soup = BeautifulSoup(r.text, 'html.parser')
